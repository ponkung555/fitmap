...
*For Claude Code: Read this file in full before writing anything to the repository.*

---

## PART ONE — USER GUIDE & PATCH NOTES

### What is FitMap?

FitMap is a single-file browser-based workout planning application. No installation, no account, no internet required after the initial load. Open the HTML file in any modern browser and it works. Everything — exercises, programs, your personal schedule, saved blends, session history — is stored locally in your browser.

The core idea: an anatomical body heatmap that lights up as you build your workout, so you can see at a glance which muscles you're training and which you're neglecting. Paired with a nutrition blender, a weekly schedule, and a cardio planning system with dynamic configuration.

---

### Current Version: v1.0

**File:** `workout-planner.html` — 353KB, completely self-contained.
...
#### 🏃 Cardio Activities (12 templates)
Each activity exposes configuration options relevant to that sport:
- **Walking** — pace (slow/brisk/fast/race-walk), incline slider 0–15%, duration, optional distance. Incline multiplies the MET value for accurate calorie estimates.
...
- Schedule ↔ Builder sync: loading a scheduled day into the builder shows a sync bar; you can save changes back to the schedule
...
## PART TWO — DEVELOPER DOCUMENTATION
...
**Structure:**
```
workout-planner.html (deliverable)
...
### Function Reference (139 functions)
...
// Panel state (widths in px)
// Stored in fitmap_panels_v1
```
...
**Cardio type guard pattern (critical — must always be used):**
```javascript
...
`renderCardioItem()` iterates `ex.config` array and renders each field by its `type` property: `'select'` → `<select>`, `'slider'` → `<input type="range">`, `'number'` → `<input type="number">`, `'text'` → `<input type="text">`. Each input calls `setCardioConfig(workoutIdx, key, value)` on change.
...
c = open('workout-planner.html').read()
...
- **`new Function()` for metFn evaluation** — the slider MET multiplier uses `new Function()` to evaluate the `metFn` string from the config schema. Works fine but could be replaced with a named function lookup table.
...
The single highest-impact missing feature. Each exercise in the workout builder should show "Last session: 80kg × 3×10 ↑" pulled from `sessionLog`. Requires reading the most recent log entry that contains that `exId` and displaying the previous weight/sets/reps inline above the inputs. Motivates consistent use.

**2. "Today's Session" Bar**
A persistent strip at the top of the workout builder showing today's scheduled session name (if one is assigned), with a single "Load" button. Bridges the gap between the schedule and the builder. Currently users must open the full schedule modal to load today's session.

**3. Visual Design Depth Pass**
The design system tokens are well-structured but the rendered result is visually flat. Needed: subtle box shadows on cards, slightly different background levels between nested surfaces, more pronounced hover states, interactive elements that feel physically pressable. No functionality changes — pure CSS.

### Medium Priority
**4. HIIT Builder (Part 3 of Cardio)**
A dedicated interval design tool accessible from the Analysis tab when a cardio session is loaded. User sets work interval duration, rest interval duration, number of rounds, and chooses a base exercise. App renders a visual timeline of work/rest blocks with estimated total duration and calorie burn. Preset protocols: Tabata (20/10), EMOM, 30/30, Pyramid.

**5. Muscle Recovery Indicator**
Read `sessionLog` to determine which muscles were trained in the last 48 hours. Show a recovery state overlay on the body heatmap — muscles still recovering shown in a cool blue with a clock icon. Warn in the Analysis tab when the current session re-trains a muscle that hasn't had 48h recovery.

**6. Workout Streak Tracker**
Count consecutive weeks with 3+ completed sessions from `sessionLog`. Show a 🔥 streak counter on the topbar. Simple to implement, strong habit-forming effect.

### Lower Priority
**7. Mobile Layout**
Responsive breakpoints below 1000px. The 3-column grid collapses into a tab-based layout: Library tab, Workout tab, Analysis tab at the bottom. The body figure moves to a collapsed panel with expand-on-tap. Significant CSS work, no JS changes needed.

**8. Share Workout via URL**
Base64-encode the current `workout` array into a URL parameter. Anyone opening that URL gets the workout loaded automatically. Zero backend required. Useful for trainers sharing sessions with clients, or for sharing blends on social media.

**9. Print / Export Workout**
A `window.print()` triggered layout showing today's workout as a clean, ink-friendly sheet — exercise name, sets, reps, weight, notes, warm-up, cool-down. CSS `@media print` stylesheet. Useful for people who prefer paper in the gym.

**10. Blend of the Day**
On opening the Blender, if it's empty, auto-suggest a featured blend based on the day of the week or the most recent workout. A small personalised delight that makes the app feel alive.

---

*End of documentation. File: `FITMAP_PROJECT.md`*
